...
istackSwap (i1 : i2 : is) = Just (i2 : i1 : is)
...
istackRot (a : b : c : rest) = Just (c : a : b : rest)
istackRot _ = Nothing
...
eval (w : ws) state@(istack, dict, out) =
  case dlookup w dict of
    ...
    otherwise ->
      -- reset IStack and add error message
...
cstackElse (("if", kif) : rest) = Just $ ("else", id) : ("if", kif) : rest
...
transUntilLoop :: Transition -> ForthState -> ForthState
